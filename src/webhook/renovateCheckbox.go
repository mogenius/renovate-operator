...
func isRenovateContent(description string) bool {
	...
	// merge requests / pull requests created by Renovate contain these specific comments
	if strings.Contains(description, "<!-- rebase-check -->If you want to rebase/retry this") {
		return true
	}
	if strings.Contains(description, "This PR has been generated by [Renovate Bot]") {
		return true
	}

	// Dependency Dashboards created by Renovate contain these specific comments
	if strings.Contains(description, "This issue lists Renovate updates and detected dependencies.") {
		return true
	}
