...
func (worker *tokenRenewalWorker) doWork() {
	worker.mu.Lock()
	jobs := make([]*api.RenovateJob, 0, len(worker.activeWorker))
	keys := make([]string, 0, len(worker.activeWorker))
	...
	for i, job := range jobs {
		worker.logger.Info("renewing github app token for job", "job", keys[i])
		err := worker.handleJob(worker.ctx, job)
		if err != nil {
			worker.logger.Error(err, "failed to renew github app token for job", "job", keys[i])
		}
	}
}
