...
// traceparentEnvVar returns a TRACEPARENT env var for W3C trace context
// propagation into Renovate Jobs. This is injected independently of OTEL_*
// forwarding so that trace correlation is available even when full OTel export
// is not configured for Jobs.
func traceparentEnvVar(traceparent string) []v1.EnvVar {
	if traceparent == "" {
		return nil
	}
	return []v1.EnvVar{{Name: "TRACEPARENT", Value: traceparent}}
}
