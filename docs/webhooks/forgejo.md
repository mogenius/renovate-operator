...
| `tokenSecretRef`     |    no    |                                | Reference to a Kubernetes secret containing a Forgejo API token. This token must belong to a user with admin access on the target repos.                                                                                                        |
